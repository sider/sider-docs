...
$ docker run --rm \
    ...
    sider/ecr-image-puller {aws_account_id}.dkr.ecr.{region}.amazonaws.com/sideci_onprem:release-202002.2
$ docker run --rm \
    -v /var/run/docker.sock:/var/run/docker.sock \
    -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID \
    -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY \
    sider/ecr-image-puller {aws_account_id}.dkr.ecr.{region}.amazonaws.com/catpost_onprem:release-202002.2
```
